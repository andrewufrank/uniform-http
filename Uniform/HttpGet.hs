...
makeHttpGet7 :: Bool -> Text  -> [(Text,Text)] -> Text -> ErrIO  Text
-- make a GET request  -- url is given as text (
-- call and return with text
makeHttpGet7 debugNLPrequest dest vars mimetype  = do
        when debugNLPrequest $ putIOwords ["makeHttpGet7", showT dest]
--        let  uri = if null vars then dest
--                    else  makeAbsURI (showT dest <> "/?" <>  urlEncodeVarsT vars)
--
--        when debugNLPrequest $ putIOwords ["makeHttpPOST7 uri", showT uri]
...
-- testing makeHttpGet7
test_get71 = do
    r <- runErr $ makeHttpGet7 True localhostTextFile [] "text/plain"

    assertEqual (Right "test af1\n") ( r)
